...
async function checkDatabase() {
  console.log("🔍 Checking Xata Database Contents\n");
  console.log("=".repeat(80));

  try {
    // 1. Count Sales records
    console.log("\n📊 1. SALES TABLE:");
    ...
    if (sales.length > 0) {
      ...
      sales.slice(0, 3).forEach((record, i) => {
        ...
        console.log(`     Shopper Name: ${record.shopper_name || 'N/A'}`);
        console.log(`     Shopper Link: ${record.shopper?.name || 'N/A'}`);
        console.log(`     Status: ${record.status || 'N/A'}`);
      });
    }

    // 2. Count legacy_trades records
    ...
    if (legacyTrades.length > 0) {
      ...
      legacyTrades.slice(0, 3).forEach((record, i) => {
        ...
        console.log(`     Client: ${record.raw_client || 'N/A'}`);
        console.log(`     Supplier: ${record.raw_supplier || 'N/A'}`);
      });
    }
